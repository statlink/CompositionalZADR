zad.mle <- function(y) {
  ...
  patterns <- cbind(1 - unique(za), freq/n)
  colnames(patterns)[D + 1] <- "percentages"
  list(loglik = -qa$value + con, phi = phi, mu = mu, patterns = patterns, runtime = runtime)
}
