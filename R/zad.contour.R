zad.contour <- function(phi, mu, n = 100, x = NULL, cont.line = FALSE) {
  nam <- c("X1", "X2", "X3")
  ...
  can <- CompositionalZADR::dzad(wa, phi, mu, logged = FALSE)
  ...
  filled.contour(x1, x2, mat,
                 ...
                 plot.axes = {
                   ...
                   # Add contour lines
                   if ( cont.line ) {
                     contour(x1, x2, mat, pt = "s", col="black", nlevels = 10, labcex = 0.8,
                             lwd = 1.5, add = TRUE)
                   }
                 },
                 key.axes = {axis(4, col = "black")},
                 xlab = "",  ylab = "", xlim = c(-0.1, 1.1), ylim = c(-0.1, 1.1) )
}
